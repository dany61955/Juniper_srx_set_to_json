...
ACTION_UID_MAP = {
    "6C488338-8EEC-4103-AD21-CD461AC2C472": "Accept",  # Replace with actual Accept UID
...
def load_objects(file_path):
    ...
    for obj in objects:
        ...
        if obj_type == "host":
            ...
            icmp_type = obj.get('icmp-type', '?')
            obj_dict[obj_uid] = {"type": "service", "name": obj_name, "value": f"ICMP6 {icmp_type}" if icmp_type != '?' else obj_name}
        elif obj_type == "service-rpc":
            program_number = obj.get('program-number', '?')
            obj_dict[obj_uid] = {"type": "service", "name": obj_name, "value": f"RPC {program_number}" if program_number != '?' else obj_name}
        elif obj_type == "address-range":
            start = obj.get('ipv4-address-first', '?')
            end = obj.get('ipv4-address-last', '?')
            ...
            obj_dict[obj_uid] = {"type": "host", "name": obj_name, "value": ip if ip != '?' else obj_name}
        elif obj_type == "interoperable-device":
            ip = obj.get('ipv4-address', '?')
            obj_dict[obj_uid] = {"type": "device", "name": obj_name, "value": ip if ip != '?' else obj_name}
    ...
    def resolve_group_members(group_obj, processed=None, is_service_group=False):
        ...
        valid_network_types = ["host", "network", "range", "group", "address-range", "checkpoint-host", "interoperable-device"]
        valid_service_types = ["service-tcp", "service-udp", "service-icmp", "service-icmp6", "service-rpc", "service-other", "service-group"]
        ...
        for member_uid in members:
            ...
            member_obj_name = member_obj.get("name", member_uid)
            
            # Check if member type is valid for the group type
            if is_service_group and member_obj_type not in valid_service_types:
                print(f"Warning: Invalid member type '{member_obj_type}' in service-group '{group_obj.get('name')}'. Member name: {member_obj_name}")
                continue
            elif not is_service_group and member_obj_type not in valid_network_types:
                print(f"Warning: Invalid member type '{member_obj_type}' in network group '{group_obj.get('name')}'. Member name: {member_obj_name}")
                continue
...
def extract_policy_data(connection, policy_name, write_files=True, objects_file=None):
    ...
    try:
        ...
        print(f"Debug: First batch response: {json.dumps(first_batch, indent=2)}")
...
def main():
    ...
    parser.add_argument('--rules-file', type=str, help='Path to existing rules.json file to use instead of fetching from manager')
    args = parser.parse_args()
    
    # Get input from user only if not using existing files
    if not args.rules_file:
        ...
        try:
            ...
            # Generate HTML report with timestamp and policy name
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            output_html = f"rules_{policy_name.replace(' ', '_')}_{timestamp}.html"
        ...
        print(f"Using existing rules file: {args.rules_file}")
        try:
            ...
            if args.objects_file:
                ...
                print("Error: Objects file is required when using rules file!")
                return
